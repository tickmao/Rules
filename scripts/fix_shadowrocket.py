...
replacements = {
    ...
    "国外网站 = select,手动切换,美国节点,日本节点,香港节点,台湾节点": "Outside = select,Proxy,United States,Japan,Europe",
    "国内网站 = select,DIRECT,手动切换,美国节点,日本节点,香港节点,台湾节点": "Mainland = select,DIRECT,Proxy,United States,Japan,Europe",
    "YouTube = select,手动切换,美国节点,日本节点,香港节点,台湾节点": "Google = select,Proxy,United States,Japan,Europe",
    "NETFLIX = select,手动切换,美国节点,日本节点,香港节点,台湾节点": "Gmedia = select,Proxy,United States,Japan,Europe",
    "Disney+ = select,手动切换,美国节点,日本节点,香港节点,台湾节点": "Microsoft = select,Proxy,United States,Japan,Europe",
    "Twitter = select,手动切换,美国节点,日本节点,香港节点,台湾节点": "Social = select,Proxy,United States,Japan,Europe",
    "Telegram = select,手动切换,美国节点,日本节点,香港节点,台湾节点": "Telegram = select,Proxy,United States,Japan,Europe",
    "PayPal = select,DIRECT,手动切换,美国节点,日本节点,香港节点,台湾节点": "PayPal = select,Proxy,United States,Japan,Europe",
    "网易音乐 = select,DIRECT": "AI = select,Proxy,United States,Japan,Europe",
    "苹果服务 = select,DIRECT,手动切换,美国节点,日本节点,香港节点,台湾节点": "Apple = select,DIRECT,Proxy,United States,Japan,Europe",
    "微软服务 = select,DIRECT,手动切换,美国节点,日本节点,香港节点,台湾节点": "Game = select,Proxy,United States,Japan,Europe",
    "哔哩哔哩 = select,DIRECT,手动切换,美国节点,日本节点,香港节点,台湾节点": "Emby = select,Proxy,United States,Japan,Europe",
    "游戏平台 = select,手动切换,美国节点,日本节点,香港节点,台湾节点,DIRECT": "Spotify = select,Proxy,United States,Japan,Europe",
    "AdBlock = select,REJECT": "AdBlock = select,REJECT",
    "漏网之鱼 = select,手动切换,美国节点,日本节点,香港节点,台湾节点,DIRECT": "NoAuto = select,Proxy,United States,Japan,Europe,DIRECT",
    ...
    "台湾节点 = select,policy-regex-filter=(?=.*(台|TW|(?i)Taiwan))^((?!(港|日|韩|新|美)).)*$": "# Taiwan = select,policy-regex-filter=(?=.*(台|台湾|台北|新北|🇹🇼|TW|(?i)Taiwan))^((?!(港|日|韩|新|美)).)*$",
    "欧洲节点 = select,policy-regex-filter=(?=.*(英|法|德|欧|🇬🇧|🇫🇷|🇩🇪|🇪🇺))^((?!(港|台|日|韩|新|美)).)*$": "Europe = select,policy-regex-filter=(?=.*(英|法|德|欧|🇬🇧|🇫🇷|🇩🇪|🇪🇺))^((?!(港|台|日|韩|新|美)).)*$",
    "新加坡节点 = select,policy-regex-filter=(?=.*(新加坡|狮城|狮|🇸🇬|SG|(?i)Singapore))^((?!(港|台|日|韩|美)).)*$": "# Singapore = select,policy-regex-filter=(?=.*(新加坡|狮城|狮|🇸🇬|SG|(?i)Singapore))^((?!(港|台|日|韩|美)).)*$",
    "韩国节点 = select,policy-regex-filter=(?=.*(韩|韩国|首尔|🇰🇷|KR|(?i)Korea))^((?!(港|台|日|新|美)).)*$": "# Korea = select,policy-regex-filter=(?=.*(韩|韩国|首尔|🇰🇷|KR|(?i)Korea))^((?!(港|台|日|新|美)).)*$"
}
